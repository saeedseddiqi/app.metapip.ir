...
export default function DashboardPage() {
  ...
  const run = React.useCallback(async () => {
    ...
    try {
      ...
      if (desktopMode) {
        ...
        const { getToken } = useAuth(); // only used in web branch
  ...
  }, [desktopMode, log, sessionEnabled]);
  ...
  React.useEffect(() => {
    try {
      const ok = searchParams?.get("signup") === "success";
      if (ok) {
        setShowWelcome(true);
        // Clean the URL (remove the flag) without reloading the page
