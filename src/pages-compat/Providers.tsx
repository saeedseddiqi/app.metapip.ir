...
import { useDeepLinkListener, parseClerkToken, exchangeClerkOAuthCode, openHostedSignIn } from "@/lib/auth/deepLink";
import { supabase, configureSupabaseFromRuntime, configureSupabaseForDesktop } from "@/lib/supabase";
...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // if already have a secure token, skip
        let hasSecure = false;
        try {
        ...
        if (hasSecure) return;
        // if already have supabase session, skip
        try {
  ...
  // Bootstrap to run hooks and listen for deep links
          ...
          <div className="min-h-[40vh] flex items-center justify-center" dir="rtl">
            <div className="p-4 rounded-lg border border-red-300 bg-red-50 dark:bg-red-950/30 text-sm max-w-xl">
              ...
              <ul className="list-disc pr-5 space-y-1 opacity-90">
                <li>در سرویس envs مقدار <code>CLERK_PUBLISHABLE_KEY</code> را تنظیم کنید.</li>
                <li>پس از تغییر، Dev را ری‌استارت کنید یا ۶۰ ثانیه صبر کنید.</li>
